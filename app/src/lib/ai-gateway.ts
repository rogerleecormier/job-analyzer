...
// Llama 3.3 70B Instruct — the largest and most capable open-source model
// on Cloudflare Workers AI. 128K context window handles full resumes + JDs.
// Best-in-class instruction following, structured JSON output, and reasoning
// among all open-source models available on Workers AI.
...
/**
 * Call Cloudflare Workers AI (Llama 3.3 70B) with a messages array.
 * Drop-in replacement for the previous callClaude function.
...
export async function callWorkersAI(
  ...
  const result: any = await env.AI.run(WORKERS_AI_MODEL as any, {
    messages,
  ...
  // Normalise all cases to a string.
  let text: string;
  if (typeof result === "string") {
    ...
    // response field exists but isn't a string — JSON-encode it
    text = JSON.stringify(result.response);
  } else if (result && typeof result.result === "string") {
    // Some model variants use { result: string } instead of { response: string }
    text = result.result;
  } else if (result && result.result != null) {
    text = JSON.stringify(result.result);
  } else {
    // Last resort: JSON.stringify the whole result for diagnostics
...
// Keep backward-compatible alias so any future code can import either name
