...
/**
 * Generate an ATS-optimized resume PDF from structured content.
 * Uses only standard fonts and simple text layout for ATS compatibility.
 */
export async function generateResumePdf(
  ...
  function drawText(
    ...
    // Word-wrap with optional hanging indent for continuation lines
  ...
  // Hanging indent for bullet items — aligns wrapped lines under text, not under the bullet
  const bulletPrefix = "•  ";
  const bulletIndent = fontRegular.widthOfTextAtSize(bulletPrefix, 10);

  // 1. Name Header
  ensureSpace(30);
  page.drawText(content.nameHeader, {
    x: MARGIN,
    y,
    size: 20,
    font: fontBold,
  ...
  // 4. Core Competencies (enhanced version with 8 strategic buckets)
  if ("coreCompetencies" in content && content.coreCompetencies) {
  ...
  } else if ("competencies" in content && content.competencies) {
    // Fallback for older format
    drawSection("Competencies");
    for (const item of content.competencies as string[]) {
      drawText(`•  ${item}`, { x: MARGIN + 10, hangingIndent: bulletIndent });
    }
  }

  // 5. Technical Skills (enhanced version with categories)
  if ("technicalSkills" in content && Array.isArray(content.technicalSkills)) {
    ...
    for (const skillGroup of content.technicalSkills) {
      if (typeof skillGroup === "object" && "category" in skillGroup) {
        ...
        drawText(`${category}:`, { x: MARGIN + 10, font: fontBold, size: 10 });
        drawText(skills, { x: MARGIN + 25 });
      }
    }
  } else if ("tools" in content && content.tools) {
    // Fallback for older format
    drawSection("Tools");
    for (const item of content.tools as string[]) {
      drawText(`•  ${item}`, { x: MARGIN + 10, hangingIndent: bulletIndent });
    }
  }
...
export async function generateCoverLetterPdf(content: {
  ...
  // Contact Information (same as resume)
  if (content.contactInfo) {
    ...
    for (const word of contactWords) {
      ...
      const width = fontRegular.widthOfTextAtSize(test, 9);
      if (width > CONTENT_WIDTH && contactLine) {
  ...
  // Render 3 achievement bullets with hanging indent
  if (content.bullets && content.bullets.length > 0) {
    ...
    for (const bullet of content.bullets) {
      ...
      for (const word of words) {
        const test = line ? `${line} ${word}` : word;
        const width = fontRegular.widthOfTextAtSize(test, 11);
        if (width > CONTENT_WIDTH - 20 && line) {
    ...
    y -= LINE_HEIGHT - 4; // paragraph gap after bullets
  } else if (content.body) {
    // Legacy fallback: render body as paragraph
